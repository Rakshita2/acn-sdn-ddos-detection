...
def startNetwork():
    ...
    net.start()
    # hosts will be used by cleanup handler
    ...
    def cleanup():
        """Kill background processes on hosts and stop the Mininet net."""
        try:
            print("Cleaning up spawned processes on hosts...")
            # common servers/tools we start in this script
            for h in hosts:
                # try to kill common background processes started by the script
                h.cmd('pkill -f SimpleHTTPServer || true')
                h.cmd('pkill -f http.server || true')
                h.cmd('pkill -f iperf || true')
                h.cmd('pkill -f hping3 || true')
                h.cmd('pkill -f wget || true')
        except Exception:
            pass
        try:
    ...
    def _signal_handler(sig, frame):
        print('\nReceived signal {}, cleaning up and exiting...'.format(sig))
        cleanup()
        sys.exit(0)
